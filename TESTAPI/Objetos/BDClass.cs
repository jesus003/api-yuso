    ...
    public class BDClass
    {
        ...
        public void ConexionOpen()
        {
            ...
            {//Data source=172.168.1.67\\REPCASETAS;Initial Catalog=Telepeaje;User Id=sa;Password=Er3t4ihv; MultipleActiveResultSets=True
                string ConnString = "Data source=DESARROLLO-L2\\SQLEXPRESS2;Initial Catalog=latic;User Id=sa;Password=Er3t4ihv; MultipleActiveResultSets=True";
        ...
        public ObjLogin login(string usuario, string password)
        {
            ...
            if (this.AbreConexion())
            {
                try
                {
                    ...
                    while (read.Read())
                    {
                        objLogin.ContribuyenteID = read.GetString(0);
                        objLogin.UserName = read.GetString(1);
        ...
        public List<ObjListFallas> listReportes()
        {
        ...
        public DataTable listComprobantes(string id)
        {
            if (this.AbreConexion())
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_GET_COMPROBANTES", conectar);
                    SqlDataReader read;
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@ContribuyenteID", id);
                    read = cmd.ExecuteReader();
                    table.Load(read);
                    read.Close();
                    CerrarConexion();
        ...
        public DataTable listComprobantesUUID(string id,string uuid)
        {
            if (this.AbreConexion())
            {
                try
                {
                    DataTable table = new DataTable();
                    SqlCommand cmd = new SqlCommand("SP_GET_COMPROBANTES_uuid", conectar);
                    SqlDataReader read;
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@ContribuyenteID", id);
        ...
        public List<ObjListFallas> listReportesPendientes()
        {
            List<ObjListFallas> listFallas = new List<ObjListFallas>();
